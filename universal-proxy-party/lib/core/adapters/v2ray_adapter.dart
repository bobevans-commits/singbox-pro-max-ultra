...
/// Adapter for V2Ray kernel
class V2RayAdapter extends KernelAdapter {
  Process? _process;
  bool _isRunning = false;

  V2RayAdapter({required super.config});
  ...
  Future<KernelResult<void>> start() async {
    try {
      ...
      AppLogger.info('Starting v2ray from: $binaryPath');
      ...
      _process!.stdout.transform(utf8.decoder).listen((data) {
        AppLogger.debug('v2ray: $data');
      });

      _process!.stderr.transform(utf8.decoder).listen((data) {
        AppLogger.error('v2ray error: $data');
      });
      ...
      if (_process != null && _process!.exitCode.then((_) => false).timeout(
        ...
        _isRunning = true;
        AppLogger.info('v2ray started successfully');
      ...
      AppLogger.error('Failed to start v2ray: $e');
      return KernelResult.err(
        'Failed to start v2ray: $e',
        KernelErrorCode.startFailed,
        e,
      );
    }
  ...
  Future<KernelResult<void>> stop() async {
    try {
      ...
      _process = null;
      _isRunning = false;
      AppLogger.info('v2ray stopped');
      
      return KernelResult.ok(null);
    } catch (e) {
      AppLogger.error('Failed to stop v2ray: $e');
      return KernelResult.err(
        'Failed to stop v2ray: $e',
        KernelErrorCode.stopFailed,
        e,
      );
    }
  ...
  Future<KernelResult<KernelStats>> fetchStats() async {
    try {
      ...
      // TODO: Implement actual stats fetching from V2Ray API
  ...
  Future<KernelResult<bool>> validateConfig() async {
    try {
      ...
      // TODO: Add actual JSON validation for v2ray config
  ...
  Future<String?> _getBinaryPath() async {
    final binaryName = Platform.isWindows ? 'v2ray.exe' : 'v2ray';
